...
class WholeSlideImage:
    ...
    def segmentTissue(
        ...
        filter_params = dict(filter_params or {'a_t': 100, 'a_h': 16, 'max_n_holes': 8})
        exclude_ids = list(exclude_ids or [])
        keep_ids = list(keep_ids or [])
        ...
        if use_otsu:
            ...
            img_eroded = cv2.erode(img_otsu, kernel, iterations=3)
            img_otsu = cv2.dilate(img_eroded, kernel, iterations=3)
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
                    ...
                    number_contours=False, seg_display=True, annot_display=True, annotations=False):
        ...
        if not view_slide_only:
            ...
            if annotations:
                for annotation in self.contours_tumor:
                    cv2.drawContours(img, [np.array(self.scaleContourDim(annotation, scale))],
                                     -1, annot_color, line_thickness, lineType=cv2.LINE_8, offset=offset)
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        ...
        if custom_downsample > 1:
            if custom_downsample != 2:
                raise ValueError("custom_downsample currently supports only 1 or 2")
        ...
        cont_check_fn = self._build_contour_checker(
            contour_fn,
            cont,
            ref_patch_size[0],
        )
    ...
    @staticmethod
    def _build_contour_checker(contour_fn, contour, patch_size):
        if isinstance(contour_fn, Contour_Checking_fn):
            return contour_fn
        builders = {
            'four_pt': lambda: isInContourV3_Easy(
                contour=contour,
                patch_size=patch_size,
                center_shift=0.5,
            ),
            'four_pt_hard': lambda: isInContourV3_Hard(
                contour=contour,
                patch_size=patch_size,
                center_shift=0.5,
            ),
            'center': lambda: isInContourV2(
                contour=contour,
                patch_size=patch_size,
            ),
            'basic': lambda: isInContourV1(contour=contour),
        }
        if contour_fn not in builders:
            raise ValueError(f"Unknown contour checking method: {contour_fn}")
        return builders[contour_fn]()
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        if n_contours == 0:
            return None
        progress_chunk_size = max(1, math.ceil(n_contours * 0.05))
        init = True
        for idx, cont in enumerate(self.contours_tissue):
            if (idx + 1) % progress_chunk_size == 0:
        ...
        return None if init else save_path_hdf5
