...
def sample_indices(scores, k, start=0.48, end=0.52, convert_to_percentile=False, seed=1):
    ...
    if len(indices) < 1:
        ...
        return random.choice(indices, min(k, len(indices)), replace=False)
...
def sample_rois(scores, coords, k=5, mode='range_sample', seed=1, score_start=0.45, score_end=0.55, top_left=None, bot_right=None):
    ...
    if np.isscalar(sampled_ids) and sampled_ids == -1:
        return {
            'sampled_coords': np.empty((0, coords.shape[-1]), dtype=coords.dtype),
            'sampled_scores': np.empty((0,), dtype=scores.dtype),
        }
...
def DrawMapFromCoords_patch_recover(canvas, wsi_object, coords, patch_size, vis_level, output_dir, indices=None, draw_grid=True, hdf5_file_path=""):
    downsamples = wsi_object.wsi.level_downsamples[vis_level]
    if indices is None:
        indices = np.arange(len(coords))
    total = len(indices)
    ...
    h5_name = os.path.splitext(os.path.basename(hdf5_file_path))[0]
    save_dir = os.path.join(output_dir, h5_name)
    ...
    for idx in tqdm(range(total)):
        ...
        filename = f"{h5_name}_{coord[0]}_{coord[1]}.png"
...
def StitchPatches(hdf5_file_path, downscale=16, draw_grid=False, bg_color=(0,0,0), alpha=-1):
    with h5py.File(hdf5_file_path, 'r') as file:
        ...
        if Image.MAX_IMAGE_PIXELS and w*h > Image.MAX_IMAGE_PIXELS:
...
def StitchCoords(hdf5_file_path, wsi_object, downscale=16, draw_grid=False, bg_color=(0,0,0), alpha=-1, save_patch=False, patch_output_dir=None):
    wsi = wsi_object.getOpenSlide()
    w, h = wsi.level_dimensions[0]
    print('original size: {} x {}'.format(w, h))
    
    vis_level = wsi.get_best_level_for_downsample(downscale)
    vis_level2 = wsi.get_best_level_for_downsample(1)
    ...
    if save_patch:
        if patch_output_dir is None:
            patch_output_dir = os.path.join(
                os.path.dirname(hdf5_file_path),
                "saved_patches",
            )
        DrawMapFromCoords_patch_recover(
            heatmap,
            wsi_object,
            coords,
            patch_size,
...
def SamplePatches(coords_file_path, save_file_path, wsi_object, 
    ...
    random = np.random.default_rng(seed)
    indices = random.choice(np.arange(len(coords)), min(len(coords), sample_num), replace=False)
    ...
    for idx in indices:
        ...
        asset_dict = {
            ...
            'coords': np.asarray(coord)[np.newaxis, ...],
        }
